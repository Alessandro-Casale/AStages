...
public abstract class RecipeTypeClientCache<R extends AClientRestriction<R, ?, ?>> implements AClientRestrictionCache<R, RecipeType<?>> {
    private final IndexedOrderedMultiMap<RecipeType<?>, R> cache = IndexedOrderedMultiMap.create();

    @Override
    public void add(RecipeType<?> target, R restriction) {
    ...
    @Override
    public SortedSet<R> get(RecipeType<?> target) {
        return cache.get(target);
    }

    @Override
    public R find(AClientHolder holder, RecipeType<?> target) {
